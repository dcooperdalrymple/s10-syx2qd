#ifndef GLOBAL_H_
#define GLOBAL_H_

#include <stdint.h>

// Strings

char *strip_ext(char *);
char *trim_whitespace(char *);
int isfilesafe(char);

// Drive & MFM

#define CPUFREQ 72000000

uint32_t time_to_bitofs(uint32_t, uint32_t);
int mfmtobin(unsigned char *, int, unsigned char *, int, int, int);
int bintomfm(unsigned char *, int, unsigned char *, int, int);
int getbit(unsigned char * input_data,int bit_offset);
void setbit(unsigned char * input_data,int bit_offset,int state);

// LUT

#define LUT_SIZE 256

static const unsigned char LUT_ByteBitsInverterPre[LUT_SIZE] = {
	
};

static unsigned char LUT_ByteBitsInverterPost[LUT_SIZE];

void init_lut(uint8_t);
void lut_block();

// CRC16

#define CRC_BITS (4) // Width of message chunk each iteration of the CRC algorithm

uint16_t check_crc(uint8_t *buffer, uint32_t size);

void init_crc(uint8_t *high, uint8_t *low, uint8_t *table, uint16_t polynome, uint16_t initvalue);
uint16_t generate_crc_table_entry(const uint16_t index, const uint16_t bits, const uint16_t polynome);

void update_crc(uint8_t *high, uint8_t *low, uint8_t value, uint8_t *table);
void update_crc_nibble(uint8_t *high, uint8_t *low, uint8_t bin, uint8_t *table);

#endif
